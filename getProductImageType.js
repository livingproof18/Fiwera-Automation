...
function getImageType(imageName) {
    ...
    for (let ext of extensions) {
        const imagePath = path.join(__dirname, 'transparent/goat', `${imageName}.${ext}`);
...
async function main() {
    const inputFilePath = path.join(__dirname, 'goat.json');
    const outputFilePath = path.join(__dirname, 'goatFinal.json');
