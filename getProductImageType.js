...
function getImageType(imageName) {
    ...
    for (let ext of extensions) {
        const imagePath = path.join(__dirname, 'transparent/nike', `${imageName}.${ext}`);
...
async function main() {
    const inputFilePath = path.join(__dirname, 'nike.json');
    const outputFilePath = path.join(__dirname, 'nikeFinal.json');
